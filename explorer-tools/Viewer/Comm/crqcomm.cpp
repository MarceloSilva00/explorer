    ...
    : QUdpSocket(), timer(this)
{
    ...
    cout << "Simulator at " << host.toStdString() << ":" << port << endl;
    ...
    cout << "Setting address to " << host.toStdString() << endl;
...
void CRQComm::connect(void)
{
    ...
    {
		cerr << "Failure when writting <View/>" << serverAddress.toString().toStdString() << ":" << port << endl;
        exit (-1);
    }
